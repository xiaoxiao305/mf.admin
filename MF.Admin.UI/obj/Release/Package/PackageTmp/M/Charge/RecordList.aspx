<%@ Page Title=" 充值管理 》 充值记录" Language="C#" MasterPageFile="~/M/main.Master" AutoEventWireup="true" CodeBehind="RecordList.aspx.cs" Inherits="MF.Admin.UI.M.Charge.RecordList" %>
   ...
   <style type="text/css">
    ...
    <script language="javascript" type="text/javascript">
        ...
        function insertRow(o, tr) {
            ...
            addCell(tr, (o.Flag == 0) ? "<a href='javascript:;' onclick=\"showChargeOrderWin('"+o.OrderNo+"','"+o.Account+"',"+o.SumbitMoney+",'" + o.CreateDate + "',"+o.PayChannel+")\">补单</a>" : "", 10);
            return tr;
        }
        $(document).ready(function() {
            attachCalenderbox('#starttime', '#overtime', null, new Date().Format("yyyy-MM-dd"), new Date().Format("yyyy-MM-dd"));
            showTimeBox($$("time"));
            var pagerTitles = ["订单号","用户账号","提交时间","提交金额","订单状态","支付时间","支付金额","平台交易号","支付平台","联运渠道ID","补单"];
        ...
        function getChargeActive(ret)
        {
            ...
            if (ret.Res == 1)
                ...
                sTit = "充值活动已关闭";
                $("#chkClose").attr("checked", "true");
            }
            else if (ret.Res == 3)
                sTit = "充值活动尚未生效";
            else if (ret.Res == 4)
                sTit = "充值活动已结束";
            $("#stateTitle").text(sTit);
            if (ret.Res == 0)
                attachCalenderbox2('#stime', '#etime', null, new Date().Format("yyyy-MM-dd"), new Date().Format("yyyy-MM-dd"));
            else
                attachCalenderbox2('#stime', '#etime', null, ret.StartTime, ret.EndTime);
            $("#stime,#etime").datepicker({
                maxDate: 365
            });
            $("#msgtitle").text("设置充值活动状态");
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="p" runat="server">
    ...
    <div class="search">&nbsp;&nbsp;
        ...
        <input type="button" value="开启充值活动" onclick="javascript:ajax.getChargeActiveState([], getChargeActive);" class="ui-button-icon-primary oprbutton"/>
