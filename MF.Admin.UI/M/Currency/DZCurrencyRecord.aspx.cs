...
{
    public partial class DZCurrencyRecord : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    ShowMessage("参数错误");
                    Base.WriteError(ClientIP, " getuserDZCurrencyRecord(account:", Request["account"], ") err.", Request.Url.PathAndQuery);
                    return;
                }
                if (Request["account"] == null || string.IsNullOrEmpty(Request["account"].ToString()))
                {
                    ShowMessage("获取用户德州2人包间元宝信息失败,参数错误");
            ...
            {
                ShowMessage("查询德州2人包间元宝记录获取参数异常:" + ex.Message);
