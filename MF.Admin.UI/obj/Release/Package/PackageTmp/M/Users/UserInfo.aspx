<%@ Page Title=" 用户管理 》 查看用户详细信息" Language="C#" MasterPageFile="~/M/main.Master" AutoEventWireup="true" CodeBehind="UserInfo.aspx.cs" Inherits="MF.Admin.UI.M.Users.UserInfo" %>
<asp:Content ID="Content1" ContentPlaceHolderID="h" runat="server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        function setuserinfo()
        {
            ...
            else if(t == 2 && "<%=user.PhoneKey%>" != "")//解绑手机号，但已绑定安全令
            {
                $("#lblerr").text("该用户已绑定手机安全令，不能解绑手机号");
                return;
            }
...
<asp:Content ID="Content2" ContentPlaceHolderID="p" runat="server">
    ...
    <div id="uinfodetailopreate" runat="server" class="hide">
        <ul>
            <li>当前元宝数量　　　　<a href="/M/currency/CurrencyRecord.aspx?account=<%=user.Account %>">元宝变更详情</a>　　　<a href="/m/Charge/RecordList.aspx?account=<%=user.Account %>">充值记录</a>　　　<a href="/M/currency/StrongBox.aspx?account=<%=user.Account %>">二级密码</a>　　　<a href="/M/Currency/DZCurrencyRecord.aspx?account=<%=user.Account %>">德州2人包间</a></li>
            <li>当前房卡数量　　　　<a href="/M/currency/RoomCard.aspx?account=<%=user.Account %>">房卡变更记录</a></li>
            <li>当前金豆数量　　　　<a href="/M/currency/BeanRecord.aspx?account=<%=user.Account %>">金豆变更详情</a>　　　<a href="/m/currency/ExchangeRecord.aspx?account=<%=user.Account %>&uid=<%=user.ChargeId %>&type=100">兑换元宝记录</a>　　　<a href="/m/currency/ExchangeRecord.aspx?account=<%=user.Account %>&uid=<%=user.ChargeId %>&type=101">兑换其他奖品记录</a></li>
            <li>当前欢乐卡数量　　　<a href="/M/currency/HappyCardRecord.aspx?account=<%=user.Account %>">欢乐卡变更详情</a></li>
            <li>包间账号数量　　　　<a href="/M/Users/SubUserList.aspx?account=<%=user.Account %>">包间账号详情</a></li>
        </ul>
        <div class="opreate-a">
            <a href="javascript:;" onclick="showOpreateWin(1)">修改密码</a><a href="javascript:;" onclick="showOpreateWin(2)">解绑手机</a><a href="javascript:;" onclick="showOpreateWin(3)">解绑安全令</a><a href="javascript:;" onclick="showOpreateWin(4)">解除本机锁定</a><a href="javascript:;" onclick="showOpreateWin(5)">解除安全令锁定</a><a href="javascript:;" onclick="showOpreateWin(6)">冻结/解冻账号</a>
        </div>
    </div>
